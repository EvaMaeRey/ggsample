...
`r flipbookr::chunk_reveal('facet_sample_1', title = '### facet_sample')`

```{r facet_sample_1, include = F}
set.seed(1323)
library(ggplot2)
ggplot(data = cars) +
  aes(x = speed) +
...
`r flipbookr::chunk_reveal('geom_lm_sample_2', title = '### geom_lm_sample')`

```{r geom_lm_sample_2, include = F}
...
`r flipbookr::chunk_reveal('geom_point_sample_3', title = '### geom_point_sample')`

```{r geom_point_sample_3, include = F}
# using the function
set.seed(92389)
library(ggplot2)
ggplot(data = cars) +
